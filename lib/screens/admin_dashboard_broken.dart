...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  // Cache widget ها برای جلوگیری از ایجاد مجدد
  late final List<Widget> _cachedPages;

  @override
  void initState() {
    super.initState();
    // ایجاد یکباره تمام صفحات و cache کردن آن‌ها
  ...
  List<Widget> _buildAllPages() {
    print(
      '🏗️ AdminDashboard _buildAllPages() called - ایجاد یکباره تمام صفحات',
    );
    ...
    return List.generate(adminMenuItems.length, (index) {
      if (index == 0) {
        ...
        print('🔄 Creating SmsPanel instance (cached)');
        ...
        // تنظیمات (ایندکس ۱۱)
        print('🔄 Creating SettingsPage instance (cached)');
        return Directionality(
          textDirection: TextDirection.rtl,
          child: SettingsPage(),
        );
        ...
        print('🔄 Creating SimpleUpdateHistoryScreen instance (cached)');
  ...
  Widget _buildDefaultContent(int index) {
    return Center(
      child: Column(
        ...
        children: [
          Icon(adminMenuItems[index].icon, size: 80, color: Colors.grey[600]),
